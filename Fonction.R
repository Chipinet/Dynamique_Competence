...
f <- function(a,parameters) {with(parameters,1/(1+exp(-b*(a-a_s))))}
...
modele <- function(t, state, parameters){
  with(as.list(c(state, parameters)),{
    ...
    ds=B*(s+delta*p)-mus*s-(xi+lambda)*s
    ...
    dp=(1-delta)*p*B-mup*p+theta*vc[m]
...
dynamique <- function(param,dilution=NULL,t=NULL,tmax=100){
  ...
  rootfunc <- function(t, var, par){
    ...
    with(as.list(c(var, par)),{
      ...
      dlambda=df(a, par)*omega+sum(beta*deriv[-c(1,n-(1:0))])
  ...
  for (i in 1:length(dilution)){
    ...
    #pos1 : position du point d'inflexion
    pos1 <- which(attributes(out)$indroot==2)
    if (length(pos1)==1){
      valeurs[1]<-sum(c(attributes(out)$valroot[1:nbclass+1,pos1]))
      #Calcul de la dérivé au point d'inflexion
      lambda <- param$omega*f(attributes(out)$valroot[nbclass+3,pos1], param)+sum(param$beta*c(attributes(out)$valroot[1:nbclass+1,pos1]))
      derive <- (lambda+param$xi)*attributes(out)$valroot[1,pos1]-param$eta*attributes(out)$valroot[2,pos1]-param$theta*attributes(out)$valroot[nbclass+1,pos1]-param$muc*valeurs[1]
      v.res[1:3] <-c(attributes(out)$troot[pos1],valeurs[1],derive)
    }
    #pos1 : position du maximum
    pos2 <- which(attributes(out)$indroot==1)
    if (length(pos2)==1) {
      valeurs[2]<-sum(c(attributes(out)$valroot[1:nbclass+1,pos2]))
      v.res[4:5] <- c(attributes(out)$troot[pos2],valeurs[2])
    }
